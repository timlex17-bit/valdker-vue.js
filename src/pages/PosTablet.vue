<template>
  ...
  >
    ...
    >
      <img :src="flyImage" class="w-full h-full object-contain rounded-full shadow-lg" draggable="false" @error="onImgError" />
    </div>
    ...
    >
      <div class="bg-white rounded-2xl shadow-lg p-6 max-w-md w-full relative">
        ...
        <img :src="selectedProduct.image" class="w-full h-56 object-contain mb-4 rounded-xl" draggable="false" @error="onImgError" />
...
// ✅ Normalize URL jadi HTTPS (mencegah mixed-content di https hosting)
const toHttps = (url) => {
  if (!url) return ""
  return String(url).replace("http://", "https://")
}

// ✅ img fallback kalau broken link
const onImgError = (e) => {
  const img = e?.target
  if (!img) return
    ...
    .map((it) => {
      if (it && (it.name || it.qty || it.image)) {
        return {
          ...
          image: toHttps(it.image ?? it.product?.image ?? it.product?.image_url) || "/placeholder.png",
      ...
      return {
        ...
        image: toHttps(it?.product?.image ?? it?.product?.image_url) || "/placeholder.png",
...
onMounted(async () => {
  ...
  try {
    ...
    products.value = productRes.data.map((p) => ({
      id: p.id,
      name: p.name,
      image: toHttps(p.image_url) || "/placeholder.png", // ✅ pakai image_url
    ...
    categories.value = categoryRes.data.map((cat) => ({
      id: cat.id,
      name: cat.name,
      icon: toHttps(cat.icon_url) || "/placeholder.png", // ✅ pakai icon_url
    }))
  } catch (err) {
